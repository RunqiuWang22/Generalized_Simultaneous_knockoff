...
datagen<-function(seed,n1,n2,n3,N,pi,ps0, ps1, ps2, ps3, ps12, ps13, ps23,rho1,rho2,rho3,gamma1,gamma2,gamma3,sigma1,sigma2,sigma3,samesig,scale,pb=0.25,LL=3){
  ...
  group3 <- generate_X(n=n3,N=N,pi=pi,gamma=gamma3,rho=rho3,pb=pb,LL=LL,seed)$group
	...
	beta1<-c(rep(c(b0.1*ss0,b1*ss1,0*ss2,0*ss3,b12.1*ss12,b13.1*ss13,0*ss23),each=new_pi),rep(0,(p-s0-s1-s2-s3-s12-s13-s23)*new_pi))*scale
	beta2<-c(rep(c(b0.2*ss0,0*ss1,b2*ss2,0*ss3,b12.2*ss12,0*ss13,b23.2*ss23),each=new_pi),rep(0,(p-s0-s1-s2-s3-s12-s13-s23)*new_pi))*scale
	beta3<-c(rep(c(b0.3*ss0,0*ss1,0*ss2,b3*ss3,0*ss12,b13.3*ss13,b23.3*ss23),each=new_pi),rep(0,(p-s0-s1-s2-s3-s12-s13-s23)*new_pi))*scale
