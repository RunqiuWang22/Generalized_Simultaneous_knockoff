...
myest<-function(datal){
  ...
  X1Knock<-group_seq_knock(X1,group1)
  X2Knock<-group_seq_knock(X2,group2)
  X3Knock<-group_seq_knock(X3,group3)
  X4Knock<-group_seq_knock(X4,group4)
  ###expand group
  ...
  fit1<-grpreg(as.matrix(cbind(X1C,X1KnockC)),y1,group=c(egroup1,egroup1+p),penalty="grLasso",family="gaussian",nlambda=1000)
  fit2<-grpreg(as.matrix(cbind(X2C,X2KnockC)),y2,group=c(egroup2,egroup2+p),penalty="grLasso",family="binomial",nlambda=1000)
  fit3<-grpreg(as.matrix(cbind(X3C,X3KnockC)),y3,group=c(egroup3,egroup3+p),penalty="grLasso",family="gaussian",nlambda=1000)
  fit4<-grpreg(as.matrix(cbind(X4C,X4KnockC)),y4,group=c(egroup4,egroup4+p),penalty="grLasso",family="binomial",nlambda=1000)
  ...
  return(cbind(Z1,Z1tilde,Z2,Z2tilde,Z3,Z3tilde,Z4,Z4tilde))
}
...
myresult <-function(coeff,q,choose,method){
  ...
  if (choose=="intersection"){
    ...
    myselect=intersect(intersect(intersect(myselect1,myselect2),myselect3),myselect4)
  }
