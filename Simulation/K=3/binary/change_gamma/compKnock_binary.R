...
myest<-function(datal){
  ...
  fit1<-grpreg(as.matrix(cbind(X1C,X1KnockC)),y1,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
  fit2<-grpreg(as.matrix(cbind(X2C,X2KnockC)),y2,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
  fit3<-grpreg(as.matrix(cbind(X3C,X3KnockC)),y3,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
