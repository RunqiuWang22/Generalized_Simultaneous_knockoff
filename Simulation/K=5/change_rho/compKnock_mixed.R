...
myest<-function(datal){
  ...
  X4Knock<-group_seq_knock(X4,group)
  X5Knock<-group_seq_knock(X5,group)
  ...
  fit4<-grpreg(as.matrix(cbind(X4C,X4KnockC)),y4,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
  fit5<-grpreg(as.matrix(cbind(X5C,X5KnockC)),y5,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
...
myresult <-function(coeff,q,choose,method){
  ...
  if (choose=="intersection"){
    if (method=="Max"){
      ...
      W4=pmax(Z4,Z4tilde)*(-1)^(Z4<=Z4tilde)
      W5=pmax(Z5,Z5tilde)*(-1)^(Z5<=Z5tilde)
    }
    if (method=="Diff"){
      W1=Z1-Z1tilde
      W2=Z2-Z2tilde
    ...
    mythred4=knockoff.threshold(W4,fdr=q,offset=1)
    myselect4=which(W4>=mythred4)
    mythred5=knockoff.threshold(W5,fdr=q,offset=1)
    myselect5=which(W5>=mythred5)
    
    myselect=intersect(intersect(intersect(intersect(myselect1,myselect2),myselect3),myselect4),myselect5)
  }
