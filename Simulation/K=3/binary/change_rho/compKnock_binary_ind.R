...
if (choose=="grlasso"){
  fit1<-grpreg(X1,y1,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
  fit2<-grpreg(X2,y2,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
  fit3<-grpreg(X3,y3,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
...
if (choose=="lasso"){
  cvfit1<-cv.glmnet(X1,y1,family="binomial",standardize=FALSE,intercept=TRUE) 
  fit1<-glmnet(X1,y1,family="binomial",standardize=FALSE,intercept=TRUE,lambda=(cvfit1$lambda.min))
  ...
  cvfit3<-cv.glmnet(X3,y3,family="binomial",standardize=FALSE,intercept=TRUE) 
  fit3<-glmnet(X3,y3,family="binomial",standardize=FALSE,intercept=TRUE,lambda=(cvfit3$lambda.min))
