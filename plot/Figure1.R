setwd("~/Library/CloudStorage/OneDrive-UniversityofNebraskaMedicalCenter/OneDrive - University of Nebraska Medical Center 2/group R_new_20231003/mixed_new_group_K=3_V2/K=3/mixed/result/")
source("FDR_power_plot.R")
tiff("Figure1.tif",width = 15, height = 10,units = 'in',res = 600)
par(mfrow = c(2, 3)) 
####figure 1 change s0
n1=1000
n2=1000
...
scale=2
vary_list=seq(0.25,0.5,by=0.05)*p
...
FDR_and_power_plot(FDRmat=FDRmat, powermat=powermat, names=names, cols=cols, pchs=pchs, alpha=q, vary_list,xlab=xlab, title='',leg_coords=c(min(vary_list),0.7),show_legend=T,labelsize=2,legsize=2,cexsize=2.3)
...
scale=2
vary_list=seq(0.05,0.2,by=0.05)*p
cols=c("black","red","blue","orange","green")
pchs=c(15,16,17,18,19)
xlab="(b) s1=s2=s3"
names=c("Generalized simultaneous","Pooling","Intersection","Individual (group_lasso)","Individual (Lasso)")
FDRmat=powermat=matrix(data=NA,nrow=length(vary_list),ncol=length(names))
for (i in 1:length(vary_list)){
  s0=round(p*ps0,0)
...
cols=c("black","red","blue","orange","green")
pchs=c(15,16,17,18,19)
xlab=expression(paste('(d) Within group correlation (',rho,')',sep=''))
names=c("Generalized simultaneous","Pooling","Intersection","Individual (group_lasso)","Individual (Lasso)")
FDRmat=powermat=matrix(data=NA,nrow=length(vary_list),ncol=length(names))
for (i in 1:length(vary_list)){
  rho1=vary_list[i]
...
cols=c("black","red","blue","orange","green")
pchs=c(15,16,17,18,19)
xlab=expression(paste('(e) Correlation ratio (',gamma,')',' (Scenario 1)',sep=''))
names=c("Generalized simultaneous","Pooling","Intersection","Individual (group_lasso)","Individual (Lasso)")
FDRmat=powermat=matrix(data=NA,nrow=length(vary_list),ncol=length(names))
for (i in 1:length(vary_list)){
  gamma1=vary_list[i]
...
cols=c("black","red","blue","orange","green")
pchs=c(15,16,17,18,19)
xlab=expression(paste('(f) Correlation ratio (',gamma,')'," (Scenario 2)",sep=''))
names=c("Generalized simultaneous","Pooling","Intersection","Individual (group_lasso)","Individual (Lasso)")
FDRmat=powermat=matrix(data=NA,nrow=length(vary_list),ncol=length(names))
for (i in 1:length(vary_list)){
  gamma1=vary_list[i]
