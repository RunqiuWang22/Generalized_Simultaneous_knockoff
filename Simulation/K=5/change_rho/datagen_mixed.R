...
datagen<-function(seed,n1,n2,n3,n4,n5,N,pi,ps0, ps1, ps2, ps3, ps4,ps5,ps12=0,ps13=0,ps14=0,ps15=0,ps23=0,ps24=0,ps25=0,ps34=0,ps35=0,ps45=0,rho1,rho2,rho3,rho4,rho5,gamma1,gamma2,gamma3,gamma4,gamma5,sigma1,sigma2,sigma3,sigma4,sigma5,samesig,scale,pb=0.25,LL=3){
  ...
  if (samesig){b12.2<-b12.1;b13.3<-b13.1;b14.4<-b14.1;b15.5<-b15.1;b23.3<-b23.2;b24.4<-b24.2;b25.5<-b25.2;b34.3<-b34.4;b35.3<-b35.5;b45.4<-b45.5}
  ...
  ss15<-sample(c(-1,1),s15,prob=c(0.5,0.5),replace=TRUE)
  ss23<-sample(c(-1,1),s23,prob=c(0.5,0.5),replace=TRUE)
  ss24<-sample(c(-1,1),s24,prob=c(0.5,0.5),replace=TRUE)
  ss25<-sample(c(-1,1),s25,prob=c(0.5,0.5),replace=TRUE)
  ss34<-sample(c(-1,1),s34,prob=c(0.5,0.5),replace=TRUE)
  ss35<-sample(c(-1,1),s35,prob=c(0.5,0.5),replace=TRUE)
  ss45<-sample(c(-1,1),s45,prob=c(0.5,0.5),replace=TRUE)
  
  ### new group after creating the dummy variable
  new_pi <- (pi*pb)*(LL-1) + pi*(1-pb)
  
  beta1<-c(rep(c(b0.1*ss0,b1*ss1,  0*ss2,  0*ss3, 0*ss4, 0*ss5,b12.1*ss12,b13.1*ss13,b14.1*ss14,b15.1*ss15,     0*ss23,    0*ss24,    0*ss25,     0*ss34,    0*ss35,    0*ss45),each=new_pi),rep(0,(p-s0-s1-s2-s3-s4-s5-s12-s13-s14-s15-s23-s24-s25-s34-s35-s45)*new_pi))*scale
  beta2<-c(rep(c(b0.1*ss0, 0*ss1, b2*ss2,  0*ss3, 0*ss4, 0*ss5,b12.2*ss12,    0*ss13,    0*ss14,     0*ss15,b23.2*ss23,b24.2*ss24,b25.2*ss25,     0*ss34,    0*ss35,    0*ss45),each=new_pi),rep(0,(p-s0-s1-s2-s3-s4-s5-s12-s13-s14-s15-s23-s24-s25-s34-s35-s45)*new_pi))*scale
  beta3<-c(rep(c(b0.1*ss0, 0*ss1,  0*ss2, b3*ss3, 0*ss4, 0*ss5,   0*ss12, b13.3*ss13,    0*ss14,     0*ss15,b23.3*ss23,    0*ss24,    0*ss25, b34.3*ss34,b35.3*ss35,    0*ss45),each=new_pi),rep(0,(p-s0-s1-s2-s3-s4-s5-s12-s13-s14-s15-s23-s24-s25-s34-s35-s45)*new_pi))*scale
  beta4<-c(rep(c(b0.1*ss0, 0*ss1,  0*ss2,  0*ss3,b4*ss4, 0*ss5,   0*ss12,     0*ss13,b14.4*ss14,     0*ss15,    0*ss23,b24.4*ss24,    0*ss25, b34.4*ss34,    0*ss35,b45.4*ss45),each=new_pi),rep(0,(p-s0-s1-s2-s3-s4-s5-s12-s13-s14-s15-s23-s24-s25-s34-s35-s45)*new_pi))*scale
  beta5<-c(rep(c(b0.1*ss0, 0*ss1,  0*ss2,  0*ss3, 0*ss4,b5*ss5,   0*ss12,     0*ss13,    0*ss14, b15.5*ss15,    0*ss23,    0*ss24,b25.5*ss25,     0*ss34,b35.5*ss35,b45.5*ss45),each=new_pi),rep(0,(p-s0-s1-s2-s3-s4-s5-s12-s13-s14-s15-s23-s24-s25-s34-s35-s45)*new_pi))*scale
...
#set.seed(1111)
#datal1=datagen(seed=1111,n1=1000,n2=1000,n3=1000,n4=1000,n5=1000,N=80,pi=4,ps0=0.3, ps1=0.05, ps2=0.05, ps3=0.05, ps4=0.05,ps5=0.05,ps12=0,ps13=0.05,ps14=0.05,ps15=0.05,ps23=0,ps24=0,ps25=0,ps34=0,ps35=0,ps45=0,rho1=0.5,rho2=0.5,rho3=0.5,rho4=0.5,rho5=0.5,gamma1=0.05,gamma2=0.05,gamma3=0.05,gamma4=0.05,gamma5=0.05,sigma1=1,sigma2=-1,sigma3=1,sigma4=-1,sigma5=1,samesig=0,scale=2,pb=0.25,LL=3)
