...
result=foreach (m=1:M, .combine=cbind)%dopar%{
	source("datagen_continuous.R")
  source("group_sequential.R")
	source("compKnock_continuous.R")
	source("compKnockStack_continuous.R")
  seed=iniseed+m
  set.seed(seed)
...
write.csv(result,paste("Continuous_result=",as.character(n1),"n2=",as.character(n2),"n3=",as.character(n3),"N=",as.character(N),"pi=",as.character(pi),"s0=",as.character(s0),"s1=",as.character(s1),"s2=",as.character(s2),"s3=",as.character(s3),"s12=",as.character(s12),"s13=",as.character(s13),"s23=",as.character(s23),"rho1=",as.character(rho1),"rho2=",as.character(rho2),"rho3=",as.character(rho3),"gamma1=",as.character(gamma1),"gamma2=",as.character(gamma2),"gamma3=",as.character(gamma3),"sigma1=",as.character(sigma1),"sigma2=",as.character(sigma2),"sigma3=",as.character(sigma3),"samesig=",as.character(samesig),"q=",as.character(q),"scale=",as.character(scale),".csv",sep=""))
