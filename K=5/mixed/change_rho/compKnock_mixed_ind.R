...
myknockoff <- function(datal){
  ...
  data4<-data[which(data$C==4),]
  data5<-data[which(data$C==5),]
  ...
  X5C <- model.matrix( ~ ., X5)[, -1]
  X5KnockC <- model.matrix( ~ ., X5Knock)[, -1]
  ...
  data5X=cbind(y5,as.matrix(cbind(X5C,X5KnockC)))
  
  return(list(data1X=data1X,data2X=data2X,data3X=data3X,data4X=data4X,data5X=data5X,group=group))
}
...
ffit = function(datal, choose){
  ...
  if (choose=="grlasso"){
    ...
    fit2<-grpreg(X2,y2,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
    fit3<-grpreg(X3,y3,group=c(group,group+p),penalty="grLasso",family="gaussian",nlambda=1000)
    fit4<-grpreg(X4,y4,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
    fit5<-grpreg(X5,y5,group=c(group,group+p),penalty="grLasso",family="binomial",nlambda=1000)
    ...
    Z5=abs(generateZ(fit5)[(1:p)])
    Z5tilde=abs(generateZ(fit5)[p+(1:p)])
  ...
  if (choose=="lasso"){
    ...
    cvfit5<-cv.glmnet(X5,y5,family="binomial",standardize=FALSE,intercept=TRUE) 
    fit5<-glmnet(X5,y5,family="binomial",standardize=FALSE,intercept=TRUE,lambda=(cvfit5$lambda.min))
    tmp5=as.numeric(coef(fit5))[-1]
    ...
    Z4=abs(tmp4[1:N])
    Z4tilde=abs(tmp4[N+1:N])
    Z5=abs(tmp5[1:N])
    Z5tilde=abs(tmp5[N+1:N])
    
  }
  
  return(cbind(Z1,Z1tilde,Z2,Z2tilde,Z3,Z3tilde,Z4,Z4tilde,Z5,Z5tilde))
}
...
myresult <-function(coeff,q,choose,method,p){
  ...
  if (choose=="grlasso"){
    if (method=="Diff"){
      W=(Z1-Z1tilde)*(Z2-Z2tilde)*(Z3-Z3tilde)*(Z4-Z4tilde)*(Z5-Z5tilde)
    }
